...
// start the Cloud Hypervisor VMM.
...
func (b VMCommandBuilder) Bin() string {
	if len(b.bin) == 0 {
		return defaultCloudHypervisorBin
	}

	return b.bin
}

// WithBin specifies which binary for cloud-hypervisor to use.
...
// creating the cloud-hypervisor exec.Command.
...
// os.Stdout in the cloud-hypervisor exec.Command.
...
// os.Stderr in the cloud-hypervisor exec.Command.
...
// os.Stdin in the cloud-hypervisor exec.Command.
...
// Build will build a cloud-hypervisor command using the specific arguments
